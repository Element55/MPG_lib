...
@objc(mpg_libViewManager)
class mpg_libViewManager: RCTViewManager, AVCapturePhotoCaptureDelegate {
    var currentView:mpg_libView?
    //MARK: RCTViewManager key methods
    override func view() -> mpg_libView {
        let newView = mpg_libView()
        currentView = newView
        return newView
    }
